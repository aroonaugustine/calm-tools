<?php
/**
 * CALM Web View — v2.6.2-i18n-courses
 * Public “View in browser” renderer for mailer links.
 *
 * Changes in 2.6.2:
 * - Adds localized 'completeThanks' paragraph shown ONLY when all courses are complete.
 * - Reminder ('footerHint') remains ONLY for incomplete.
 *
 * Features
 * - Renders the same progress that the mailer uses (live from LearnDash)
 * - Language switch via ?lang=en|zh|id|vi|si
 * - Course title translation map for known titles (EN→ZH/ID/VI/SI)
 * - Optional whitelist: set $COURSE_WHITELIST = [] to show all the user’s enrollments
 *
 * URL examples (public):
 ...
 *
 * Supported identity params:
 *   - login=username
 *   - OR _se=base64url(user_email)
 *
 * Note: This view is read-only and does not send mail.
 */
...
// If you want to restrict to a subset of courses, list IDs here.
// To show ALL enrollments in the view, set this to [].
$COURSE_WHITELIST = [2871,4216,2869,3027]; // or []

/* -----------------------------
   I18N: Course title translations
   Key: the canonical English title as it appears in WP (case-insensitive match via normalize)
------------------------------ */
...
/* -----------------------------
   I18N: Static strings per language (includes completeThanks)
------------------------------ */
...
$L = [
  'en' => [
    ...
    'completeThanks'=> 'These courses are vital to maintaining our high compliance and information-security standards, so thank you for keeping everything up to date.',
  ...
  'zh' => [
    ...
    'completeThanks'=> '这些课程对维持我们高标准的合规与信息安全至关重要，感谢您及时完成所有更新。',
  ...
  'id' => [
    ...
    'completeThanks'=> 'Kursus-kursus ini sangat penting untuk menjaga standar kepatuhan dan keamanan informasi kami yang tinggi, jadi terima kasih telah menjaga semuanya tetap terkini.',
  ...
  'vi' => [
    ...
    'completeThanks'=> 'Các khóa học này rất quan trọng để duy trì tiêu chuẩn tuân thủ và an ninh thông tin cao của chúng ta, vì vậy xin cảm ơn bạn đã cập nhật mọi thứ kịp thời.',
  ...
  'si' => [
    ...
    'completeThanks'=> 'අපගේ ඉහළ අනුකූලතාව සහ තොරතුරු-ආරක්ෂක ප්‍රමිතීන් පවත්වා ගැනීම සඳහා මෙම පාඨමාලා අත්‍යවශ්‍ය වේ, එබැවින් සියල්ල යාවත්කාලීනව තබා ගැනීම ගැන ඔබට ස්තූතියි.',
...
function norm_title(string $s): string {
  // normalize to compare keys (lowercase, single spaces)
...
function ld_user_course_ids_full($user_id): array {
  ...
  if (function_exists('learndash_get_users_group_ids') && function_exists('learndash_group_enrolled_courses')) {
    ...
    foreach ($group_ids as $gid) {
      $g = (array) learndash_group_enrolled_courses($gid);
...
$consider_ids = !empty($COURSE_WHITELIST) ? array_values(array_intersect($user_ids, $COURSE_WHITELIST)) : $user_ids;
...
<div class="card">
  ...
  <?php else: ?>
    <p class="footer"><?=htmlspecialchars($Lx['completeThanks'])?></p>
  <?php endif; ?>
