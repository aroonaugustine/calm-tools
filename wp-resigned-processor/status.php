...
function run_dirs(): array {
  $dirs = @glob(LOG_BASE . '/resigned_*', GLOB_ONLYDIR) ?: [];
  rsort($dirs);
  return $dirs;
}
...
if ($id === '') {
  ...
  <!doctype html>
  <meta charset="utf-8">
  <title>Resigned Processor — Runs</title>
  <style>
...
<div class="row">
  <?php if (is_file($matched)): ?>
    <a class="btn" href="csv.php?id=<?=urlencode($id)?>&t=matched">Download matched_done.csv</a>
  <?php else: ?>
    <span class="pill">matched_done.csv (pending)</span>
  <?php endif; ?>

  <?php if (is_file($unmatch)): ?>
    <a class="btn" href="csv.php?id=<?=urlencode($id)?>&t=unmatched">Download unmatched_skipped.csv</a>
  <?php else: ?>
...
<?php if ($meta): ?>
  ...
  <table class="meta">
    ...
    <tr><td class="hdr">Match mode</td><td><code><?=h($meta['match_mode'] ?? 'strict')?></code></td></tr>
    <tr><td class="hdr">Batching</td><td>size=<code><?=h((string)($meta['batch_size'] ?? 0))?></code>, delay_ms=<code><?=h((string)($meta['batch_delay_ms'] ?? 0))?></code></td></tr>
    <tr><td class="hdr">Args</td><td><code><?=h(implode(' ', array_map('strval', $meta['args'] ?? [])))?></code></td></tr>
  </table>
<?php endif; ?>
